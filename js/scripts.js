...
function cleanText() {
  ...
  cleanText = raw.replace(/\s{2,}/g, " ").replace(/[^a-zA-Z ]/g, "");
  return cleanText;
}
...
function printWordsToPage() {
  ...
  for (i = 0; i < sortedArray.length; i++) {
    if (sortedArray[i][0] == "" ||
      (STOPWORDS.includes(sortedArray[i][0]) &&
        document.getElementById("no-stop-words-toggle").checked == true)) {
      continue
    } else {
